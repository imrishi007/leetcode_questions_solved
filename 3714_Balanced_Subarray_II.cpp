...
public:
    ...
    int duo(const string& s, char c1, char c2){
        ...
        for(int i = 0; i < (int)s.size(); i ++){
            //If we encounter a string which is neither of both then reset the counter
            if(s[i] != c1 && s[i] != c2){
                pos.clear();
                pos[0] = i;
                delta = 0;
            ...
            //first word encountered - increase delta or decrease if we encounter the second one
            if(s[i] == c1){
                delta++;
            }
            else{
